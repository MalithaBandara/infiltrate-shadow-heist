...
// Same interface/logic as src/GameSfxOutput.kt + src@android/GameSfxOutput.android.kt, but
...
// a single plain implementation is all this module needs. This is the one that actually ships -
// see GameSfxOutput's own doc comment in src/ for why it exists at all.
...
object AndroidGameSfxOutputState {
    ...
    /**
     * Called from MainActivity's onPause/onResume - the mixer thread/AudioTrack described on
     * [AndroidGameSfxOutput] runs forever once started, with no lifecycle awareness of its own,
     * so without this it kept playing after leaving the app (backgrounding, not just an explicit
     * quit-to-menu, which already goes through stopBgMusic/stopMusic - this is the "the whole
     * engine" version of that for when the app itself is no longer in the foreground at all).
     */
    fun pauseEngine() {
...
private class AndroidGameSfxOutput : GameSfxOutput, PausableAudioEngine {
    ...
    /**
     * Called via [AndroidGameSfxOutputState.pauseEngine]/`resumeEngine` from MainActivity's
...
fun getGameSfxOutput(): GameSfxOutput? = AndroidGameSfxOutput()
